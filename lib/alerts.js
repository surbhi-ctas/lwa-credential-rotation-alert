...
function actionStepsHtml(severityKey, overdue) {
  if (severityKey === 'critical' || overdue) {
    return `
      <ol style="margin:8px 0 0;padding-left:20px;color:#111827;font-size:14px;line-height:1.6">
        ...
        <li><strong>Update</strong> <code>client_secret</code> in your store record (OMS <code>tbl_store_details</code>).</li>
  ...
  if (severityKey === 'warning') {
    return `
      <ol style="margin:8px 0 0;padding-left:20px;color:#111827;font-size:14px;line-height:1.6">
        ...
        <li>Rotate LWA client secret in Amazon, then update OMS credentials.</li>
  ...
  return `
    <ol style="margin:8px 0 0;padding-left:20px;color:#111827;font-size:14px;line-height:1.6">
      ...
      <li>After rotation, update next rotation date in OMS.</li>
    </ol>`;
}

function actionStepsText(severityKey, overdue) {
  if (severityKey === 'critical' || overdue) {
    return [
      'PRIORITY ACTIONS (CRITICAL):',
      '1) Rotate LWA client secret in Amazon NOW.',
      '2) Update client_secret in OMS tbl_store_details.',
  ...
  if (severityKey === 'warning') {
    return [
      ...
      '2) Rotate secret in Amazon and update OMS.',
  ...
  return [
    ...
    '3) After rotation, update next date in OMS.'
  ].join('\n');
}
...
function buildTextBody(evaluation, lines) {
  ...
  return [
    ...
    ...lines,
    '',
    actionStepsText(severityKey, overdue),
    '',
        ...
        <table role="presentation" width="100%" style="max-width:600px;background:#ffffff;border-radius:8px;overflow:hidden;border:1px solid #e5e7eb">
          <tr>
            <td style="background:${meta.color};color:#ffffff;padding:16px 20px">
              ...
              <div style="font-size:20px;font-weight:700;margin-top:4px">LWA Client Secret Rotation Alert</div>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:20px">
              ...
              <div style="margin-top:20px;padding:14px 16px;border-radius:6px;border:1px solid ${meta.border};background:${meta.bg}">
                ...
                ${actionStepsHtml(severityKey, overdue)}
              </div>
